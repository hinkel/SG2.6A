...
const clivalue_t valueTable[] =
{
    ...
    { "rc_flpsp",                  VAR_UINT8,  &cfg.rc_flpsp,                    0,          1, 1 },
    ...
    { "acc_ilpfhz",                VAR_FLOAT,  &cfg.acc_ilpfhz,                  0,         50, 1 },
    ...
    { "gps_ph_expo",               VAR_UINT8,  &cfg.gps_ph_expo,                 0,         99, 1 },
    ...
    { "looptime",                  VAR_UINT16, &cfg.looptime,                 3000,       9000, 1 },
...
static void cliDebug(char *cmdline)
{
    ...
    if (sensors(SENSOR_MAG))
    {
        ...
        printf("\r\nRuntime\r\n");
        for (i = 0; i < 3; i++) printf("%c Gain*1000: %d\r\n", k++, (int32_t)(magCal[i] * 1000));      
...
static void cliAuxset(char *cmdline)
{
    ...
    ptr = strchr(ptr, ' ') + 1;
    AuxChNr = atoi(ptr);
    if (AuxChNr > cfg.rc_auxch || !AuxChNr || ItemID >= CHECKBOXITEMS)
    {
        cliErrorMessage();
        return;
    }
...
#define RcEndpoint 50
    ...
    static uint8_t   input, lastinput, brake, brakeval, speeduptimer;
    ...
    delay(2000);
    LCDclear();
    exitLCD = brake = brakeval = speeduptimer = 0;
    DatasetNr = 0;
    LCDline1();
    ...
    while (!exitLCD)
    {
        ...
        if ((int32_t)(timetmp - rctimer) >= 0)                          // Start of 50Hz Loop
        {
            ...
            {
                if (rcData[YAW] > (cfg.rc_max - RcEndpoint)) exitLCD = 1;  // Quit don't save
                if (rcData[YAW] < (cfg.rc_min + RcEndpoint)) exitLCD = 2;  // Quit and save
            }
