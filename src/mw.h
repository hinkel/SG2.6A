...
#define  VERSION  211
#define  FIRMWARE  "Harakiri10 Summer Games 2.6A" __DATE__ " / " __TIME__
...
typedef struct config_t
{
    ...
    uint8_t  rc_flpsp;                      // Flip support will turn Motors to idle when upside down in acro or horizon mode
    ...
    float    acc_ilpfhz;                    // ACC lowpass in Hz for Acc GPS INS
    uint8_t  gps_ph_minsat;                 // Minimal Satcount for PH, PH on RTL is still done with 5Sats or more
    uint8_t  gps_ph_expo;                   // 1 - 99 % defines the actual Expo applied during Ph in range of the expotub
