...
- Added rc_flpsp = x (Default: Disable 0, Enable: 1) Flip support will turn Motors to idle (esc_min) when upside down in acro or horizon mode.
...
-- acc_lpf  = 100 deleted and repaced by acc_lpfhz  = 0.536 (that is equivalent to the former "100" based on 3ms cycletime)
-- acc_ilpf =  10 deleted and repaced by acc_ilpfhz = 5.895 (that is equivalent to the former " 10" based on 3ms cycletime)
...
- gps_ph_abstub & nav_slew_rate deleted and replaced by gps_ph_expo. It's what you think an expocurve [0-99%, 0 disables] for the PH gps auto steering.
...
- Decode only one mwii/mavlink command in one cycle when looptime is set low or the code produces timespikes greater than the predefined looptime.
  This is just done now if one fine day the looptime exceeds 3ms (looptime has new lower limit: 3000) so you will not notice that.
  If looptime gets excessive the usb/serial is reduced to 10Hz and only one command is processed from buffer.
  Serial gets choppy then and may produce packetloss, but that usb serial is not vital.
...
- Inserted DEBUGMODE as #define in the code for some sensors. Reachable via CLI (needs recompile). Spits out some internal calibration data, shows unused/free bytes in eeprom with status command.
...
- Deleted gps_ph_abstub replaced with gps_ph_expo
- Deleted nav_slew_rate replaced with gps_ph_expo
- Deleted gps_phase thought it is useful but not needed...
- Deleted acc_lpf repaced by acc_lpfhz
- Deleted acc_ilpf repaced by acc_ilpfhz
